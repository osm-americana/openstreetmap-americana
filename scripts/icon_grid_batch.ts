...
const diff = execSync(`git diff --name-status origin/main...HEAD -- ${ICONS_DIR}/`)
  .toString()
  .split("\n")
  .filter(Boolean)
  .map(line => line.split("\t"))
  .filter(([status, file]) => (status === "A" || status === "M") && file.endsWith(".svg"))
  .map(([, file]) => file);
...
if (diff.length === 0) {
  ...
  for (const icon of diff) {
    const outPng = path.join(OUTPUT_DIR, path.basename(icon, ".svg") + "_preview.png");
    try {
